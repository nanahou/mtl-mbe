...
class WaveReader(Reader):
    ...
    def __init__(self, wav_scp, sample_rate=None, normalize=True, use_mulaw=True):
...
def mu_law_decode_4tensor(output, quantization_channels=256):
    '''Recovers waveform from quantized values.'''
    mu = quantization_channels - 1
    # Map values back to [-1, 1].
    #signal = 2 * ((output*1.0) / mu) - 1
    signal = output*1.0
    # Perform inverse of mu-law transformation.
    magnitude = (1 / mu) * ((1 + mu)**th.abs(signal) - 1)
    return th.sign(signal) * magnitude