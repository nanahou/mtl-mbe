...
class Conv1DBlock_4BWE(nn.Module):
    """
    1D convolutional block:
        Conv1x1 - PReLU - Norm - DConv - PReLU - Norm - SConv
    """
    ...
    def __init__(self,
                 ...
                 norm="cLN",
                 causal=False):
        super(Conv1DBlock_4BWE, self).__init__()
        # 1x1 conv
        self.conv1x1 = Conv1D(in_channels, conv_channels, 1)
        self.prelu1 = nn.PReLU()
        self.lnorm1 = build_norm(norm, conv_channels)
        ...
        self.lnorm2 = build_norm(norm, conv_channels)
        # 1x1 conv cross channel
        self.sconv = nn.Conv1d(conv_channels, conv_channels, 1, bias=True)
        # different padding way
        self.causal = causal
        self.dconv_pad = dconv_pad
...
class TCN(nn.Module):
    def __init__(self,
        ...
        for i in range(self.num_layers):
            ...
            self.encoder.append(Conv1DBlock_4BWE(echannelin[i], echannelout[i], self.filter_size_en[i],
                                              dilation=dilation_e,
                                              norm=norm_type,
                                              causal=causal))
            ...
            self.decoder.append(Conv1DBlock_4BWE(dchannelin[i], dchannelout[i], self.filter_size_de[i],
                                              dilation=dilation_d,
                                              norm=norm_type,
                                              causal=causal))
...
class TasBweTasNet(nn.Module):
    def __init__(self,
                 ...
                 non_linear="relu",
                 causal=False):
        super(TasBweTasNet, self).__init__()
        supported_nonlinear = {
            "relu": F.relu,
            "sigmoid": th.sigmoid,
            "softmax": F.softmax
...
def foo_TasBweNet():
    x = th.rand(4, 1000)
    nnet = TasBweTasNet(norm="cLN", causal=False)
    # print(nnet)
