...
from libs.audio import mu_law_decode_4tensor
...
class SiSnrTrainer(Trainer):
    ...
    def compute_loss(self, egs):
        # spks x n x S
        # ests_mid means output_enhan in the middle net
        '''
         mix is the input (noisy re16k)
         ref1 is the target (clean re16k)
         ref2 is the target (clean true 16k)

         ests means final output (match ref2)
         ests_mid means output_enhan in the middle net (match ref1)
        '''
        ests, ests_mid = th.nn.parallel.data_parallel(
            self.nnet, egs["mix"], device_ids=self.gpuid)
        # spks x n x S
        refs1 = egs["ref1"]
        refs2 = egs["ref2"]

        def sisnr_loss(ests, ests_mid, refs1, refs2):
            return sum([self.sisnr(ests[0], refs2[0]) + self.sisnr(ests_mid[0], refs1[0])])

        # P x N
        N = egs["mix"].size(0)

        sisnr_mat = th.stack([sisnr_loss(ests, ests_mid, refs1, refs2)])
